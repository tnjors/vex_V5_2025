...
void genericDrive(OrientationEnum orientation) {
  ...
  if (orientation == LEFT) {
    ...
    pros::delay(1300);  // RING_EJECT_DELAY // 1500
    intake.move(0);
  ...
  chassis.pid_drive_set(GENERIC_DRIVE_TO_LOADER + 2, 110);
  chassis.pid_wait_quick_chain();

  pros::delay(PICKUP_AUTOS - 250);  // pickup delay for loader !!!! 300 50

  // Move to far goal

  // drive(-30); //-30

  chassis.pid_drive_set(-DRIVE_DISTANCE_CYCLE2, 120);
  chassis.pid_wait_quick_chain();
...
void skills() {
  ...
  turnRel(45);
  drive(17.7);
  turnRel(89);  // compensating for drift along the lng goal 88
  drive(76);    // 83
...
void drive_left() {
  ...
  outtake.set(false);
  hoarder.set(false);  // switched true

  chassis.pid_drive_constants_set(8, 0, 18.0);

  chassis.pid_drive_set(-25.5, DRIVE_SPEED);  //-31
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_wait_quick_chain();
  chassis.drive_set(50, 50);  // Maintain light pressure against the loader
  ...
  // drive(23);  // 16
  chassis.pid_drive_set(21, DRIVE_SPEED);  // 80 spd
...
void drive_right() {
  ...
  // drive(19);  // 16
  chassis.pid_drive_set(16, 80);  // 18
  chassis.pid_wait_quick();
  ...
  chassis.pid_drive_set(-42, DRIVE_SPEED);  // 27
  chassis.pid_wait_quick_chain();
  ...
  chassis.pid_drive_set(-17, DRIVE_SPEED);  // 27
  chassis.pid_wait_quick_chain();
...
void drive_swp() {
  ...
  outtake.set(false);
  hoarder.set(false);//fkipped true

  chassis.pid_drive_constants_set(8, 0, 18.0);

  chassis.pid_drive_set(-25.5, DRIVE_SPEED);
  chassis.pid_wait_quick_chain();
